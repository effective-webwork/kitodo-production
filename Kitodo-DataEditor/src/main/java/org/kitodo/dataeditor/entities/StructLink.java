...
public class StructLink extends MetsType.StructLink {
    ...
    /**
     * Returns a list of ids of logical div objects which are linked by a given
     * physical div.
     *
     * @param physicalDiv
     *            The physical div element.
     * @return The list of ids.
     */
    public List<String> getLogicalDivIdsByPhysicalDiv(DivType physicalDiv) {
        List<SmLink> smLinks = getSmLinks();
        List<String> ids = new ArrayList<>();
        for (SmLink smLink : smLinks) {
            if (Objects.equals(smLink.getTo(), physicalDiv.getID())) {
                ids.add(smLink.getFrom());
            }
        }
        return ids;
    }
