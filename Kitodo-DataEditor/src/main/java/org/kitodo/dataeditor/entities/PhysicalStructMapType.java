...
public class PhysicalStructMapType extends StructMapType {
    ...
    public List<DivType> createDivsByFileTypes(List<FileType> fileTypes, String physicalDivId) throws Exception {
        if (Objects.isNull(this.getDiv())) {
            this.setDiv(objectFactory.createRootDivTypeForPhysicalStructMap());
        }
        List<DivType> divTypes = this.getDivTypesByFileTypes(fileTypes);

        // only look for div with physical id if id is not empty
        if (!physicalDivId.equals("")) {
            List<DivType> boundBookChildren = this.getDiv().getDiv(); // Children of BoundBook
            for (DivType child : boundBookChildren) {
                if (child.getID().equals(physicalDivId)) {
                    child.getDiv().addAll(divTypes);
                    return divTypes;
                }
            }
        }
        if (this.getDiv().getID().equals(physicalDivId)) {
            this.getDiv().getDiv().addAll(divTypes);
            return divTypes;
        }
        throw new Exception("Partial job with id " + physicalDivId + " could not be found in physical structMap.");
    }

    private List<DivType> getDivTypesByFileTypes(List<FileType> fileTypes) {
        List<DivType> divTypes = new ArrayList<>();
        int counter = getNewId(this.getDiv(), 0) + 1;
    ...
    private int getNewId(DivType divType, int currentMaxId) {
        for (DivType child : divType.getDiv()) {
            String idString = child.getID();
            if (idString.contains("PHYS_")) {
                int currentId = Integer.parseInt(idString.replace("PHYS_", ""));
                if (currentId > currentMaxId) {
                    currentMaxId = currentId;
                }
            }
            currentMaxId = getNewId(child, currentMaxId);
        }
        return currentMaxId;
    }
    ...
    public void addPartialJob(DivType divType) {
        if (Objects.isNull(this.getDiv())) {
            this.setDiv(objectFactory.createRootDivTypeForPhysicalStructMap());
        }
        this.getDiv().getDiv().add(divType);
    }

    public int getPhysicalStructMapSize() {
        int size = 1;
        if (Objects.nonNull(super.getDiv())) {
            List<DivType> divTypes = super.getDiv().getDiv();
            if (!divTypes.isEmpty()) {
                size = getSizeOfDivTypes(divTypes, size);
            }
        }
        return size;
    }

    private int getSizeOfDivTypes(List<DivType> divTypes, int size) {
        for (DivType div : divTypes) {
            size++;
            if (!div.getDiv().isEmpty()) {
                size = getSizeOfDivTypes(div.getDiv(), size);
            }
        }
        return size;
    }
