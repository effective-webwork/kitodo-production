...
public class DivXmlElementAccess extends LogicalDivision {
    ...
    private static final String ID_PREFIX = "LOG_";
    ...
    /**
     * Workpiece to which this Object belongs. The Workpiece is used to calculate a human-readable id.
     */
    ...
    /**
     * Creates a new DivXmlElementAccess for an existing structure and takes the workpiece into account
     * to create a human-readable id for this object.
     */
    DivXmlElementAccess(LogicalDivision logicalDivision, Workpiece workpiece) {
        super(logicalDivision);
        this.workpiece = workpiece;
        if (logicalDivision instanceof DivXmlElementAccess) {
            metsReferrerId = ((DivXmlElementAccess) logicalDivision).metsReferrerId;
        } else if (Objects.nonNull(workpiece)) {
            //'unsavedIndex' parameter calculate the index of 'logicalDivision' in all newly added and unsaved structuralElements
            int unsavedIndex = workpiece.getAllLogicalDivisions()
                    .stream()
                    .filter(i -> !(i instanceof DivXmlElementAccess))
                    .collect(Collectors.toList())
                    .indexOf(logicalDivision);
            if (unsavedIndex < 0) {
                metsReferrerId = ID_PREFIX + String.format("%04d", getMaxId(workpiece) + 1);
            } else {
                metsReferrerId = ID_PREFIX + String.format("%04d", getMaxId(workpiece) + 1 + unsavedIndex);
            }
        } else {
            metsReferrerId = KitodoUUID.randomUUID();
        }
    ...
    DivType toDiv(Map<PhysicalDivision, String> physicalDivisionIDs, LinkedList<Pair<String, String>> smLinkData, Mets mets) {
        ...
        for (LogicalDivision subLogicalDivision : super.getChildren()) {
            div.getDiv().add(Objects.nonNull(workpiece)
                    ? new DivXmlElementAccess(subLogicalDivision, workpiece).toDiv(physicalDivisionIDs, smLinkData, mets)
                    : new DivXmlElementAccess(subLogicalDivision).toDiv(physicalDivisionIDs, smLinkData, mets));
        }
        return div;
    }
     ...
     * Get metsReferrerId.
     *
     * @return value of metsReferrerId
     */
    public String getDivId() {
        return metsReferrerId;
    }

    private int getMaxId(Workpiece workpiece) {
        return workpiece.getAllLogicalDivisions().stream()
                .filter(element -> element.getId().startsWith(ID_PREFIX))
                .filter(element -> StringUtils.isNumeric(element.getId().replace(ID_PREFIX, "")))
                .max(Comparator.comparingInt(o -> Integer.parseInt(o.getId().replace(ID_PREFIX, ""))))
                .map(element -> Integer.parseInt(element.getId().replace(ID_PREFIX, "")))
                .orElse(0);
    }
}
