...
public class MetadataValidation implements MetadataValidationInterface {
    ...
    public ValidationResult validate(Workpiece workpiece, RulesetManagementInterface ruleset,
        ...
        // Vecteur allows structures without media and unlinked media
        checkMedia = false;
        if (checkMedia) {
        ...
        for (PhysicalDivision physicalDivision : workpiece.getAllPhysicalDivisions()) {
            StructuralElementViewInterface divisionView = ruleset.getStructuralElementView(physicalDivision.getType(), null,
                metadataLanguage);
            results.add(checkForMandatoryQuantitiesOfTheMetadataRecursive(
                physicalDivision.getMetadata().parallelStream().collect(Collectors.toSet()),
                divisionView, physicalDivision.toString().concat(": "), translations));
            results.add(checkForDetailsInTheMetadata(
                physicalDivision.getMetadata().parallelStream().collect(Collectors.toSet()),
                divisionView, physicalDivision.toString().concat(": "), translations));
        }
