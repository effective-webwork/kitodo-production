...
public class ImageGenerator implements Runnable {
    ...
    public void removeGeneratedContent() {
        for (Subfolder subfolder : outputs) {
            Folder generatorSource = subfolder.getProcess().getProject().getGeneratorSource();
            if (!Objects.equals(generatorSource.getRelativePath().replaceFirst("\\*\\..+$", ""),
                    subfolder.getRelativeDirectoryPath().replaceFirst("\\*\\..+$", ""))) {
     ...
     */
    public List<Subfolder> determineFoldersThatNeedDerivatives(Pair<String, URI> source) {
        List<Subfolder> foldersThatNeedDerivatives = new ArrayList<>(outputs.size());
        Predicate<? super Subfolder> requiresGeneration = mode.getFilter(source.getKey());
        for (Subfolder folder : outputs) {
            // NOTE: we need to check file extension in Vecteur because multiple folders (with identical path) are configured as generator sources at once!
            if (requiresGeneration.test(folder)
                    && FilenameUtils.getExtension(source.getValue().getPath()).equals(FilenameUtils.getExtension(folder.getFolder().getPath()))) {
    ...
    public void determineSources() {
        ...
        // Hack McHacky!
        if (sourceFolder.getFolder().isCustom()) {
            for (Folder folder : sourceFolder.getProcess().getProject().getFolders()) {
                if (folder.getFileGroup().equals(sourceFolder.getFolder().getFileGroup())) {
                    continue;
                }
                if (folder.isCustom() && folder.getRelativePath().equals(sourceFolder.getFolder().getRelativePath())) {
                    Subfolder currentSubFolder = new Subfolder(sourceFolder.getProcess(), folder);
                    contents.putAll(currentSubFolder.listContents());
                }
            }
        }
    ...
    private void generateDerivative(URI sourceImage, Subfolder destinationImage, String canonical)
        ...
        boolean isCustom = imageProperties.isCustom();
        ...
        if (optionalDerivative.isPresent() && destinationImage.getFileFormat().getImageFileFormat().isPresent()) {
            ...
            imageService.createCustomImage(destinationImage.getProcess(), canonical);
        }
    }
