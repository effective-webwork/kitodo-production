...
public class WorkflowControllerService {
    public static final String RECEIVE_DIGITIZATION_JOB = "ReceiveDigitizationJob";
    public static final String RECEIVE_DIGITIZATION_JOB_DE = "Digitalisierungsauftrag empfangen";
    public static final String REQUEST_DOSSIER_FROM_VIADUC = "RequestDossierFromViaduc";
    public static final String REQUEST_DOSSIER_FROM_VIADUC_DE = "Dossier anfordern";
    ...
    public void close(Task task) throws DataException, IOException, DAOException {
        String lastState = JobService.getViaducState(task.getProcess());

        task.setProcessingStatus(TaskStatus.DONE);
        boolean isCorrection = task.isCorrection();
        task.setCorrection(false);
        Date processingTime = task.getProcessingTime();
        task.setProcessingTime(new Date());
        User user = null;
        if (!task.isTypeAutomatic() && (!task.getTitle().equals(RECEIVE_DIGITIZATION_JOB) || !task.getTitle().equals(RECEIVE_DIGITIZATION_JOB_DE)) && (!task.getTitle().equals(REQUEST_DOSSIER_FROM_VIADUC) || !task.getTitle().equals(REQUEST_DOSSIER_FROM_VIADUC_DE))) {
            user = getCurrentUser();
        }
        ...
        ServiceManager.getProcessService().refresh(task.getProcess());

        automaticTasks = new ArrayList<>();

        // add 'finished' event to PREMIS file!
        try {
            PremisHelper.addTaskEventToPremisFile(task, processingTime, null, isCorrection);
        ...
        logger.info(AuditingLogger.createProcessTaskClosedLogEntry(task));
        String currentState = JobService.getViaducState(task.getProcess());
        if (!Objects.equals(lastState, currentState)) {
            logger.info(AuditingLogger.createProcessStateChangedLogEntry(task.getProcess(), lastState, currentState));
        }

        // Update capacity if necessary: Job should be removed if it is digitized external.
        if (Objects.equals(lastState, "IN_PROGRESS") && Objects.equals(currentState, "EXTERNAL")) {
            JobService.removeJobFromUsedCapacity(task.getProcess());
        }
    }
    ...
    public void assignTaskToUser(Task task) {
        ...
        try {
            if (!this.flagWait) {
                ...
                PremisHelper.addTaskEventToPremisFile(task, null,null, task.isCorrection());
            ...
            this.flagWait = false;
        } catch (DataException | IOException | PremisException | ParserConfigurationException e) {
    ...
    public void unassignTaskFromUser(Task task) throws DataException {
        ...
        try {
            PremisHelper.addTaskEventToPremisFile(task, processingTime,null, task.isCorrection());
        } catch (IOException | PremisException | ParserConfigurationException e) {
    ...
    public void reportProblem(Comment comment, TaskEditType taskEditType) throws DataException {
        ...
        // add "finished with errors" event to PREMIS file!
        try {
            PremisHelper.addTaskEventToPremisFile(currentTask, currentTask.getProcessingTime(), comment, currentTask.isCorrection());
        } catch (IOException | ParserConfigurationException | PremisException e) {
            logger.error("Could not write PREMIS file: {}", e.getMessage());
        }
        if (currentTask.isConcurrent()) {
            for (Task task : currentTask.getProcess().getTasks()) {
                if (task.isConcurrent() && task.getOrdering().equals(currentTask.getOrdering())) {
        ...
        taskService.save(currentTask, true);
        ...
        if (Objects.nonNull(comment.getCorrectionTask())) {
            ...
            taskService.save(correctionTask, true);
    ...
    private void activateTasksForClosedTask(Task closedTask) throws DataException, IOException, DAOException {
        ...
        for (Task automaticTask : automaticTasks) {
            ...
            taskService.save(automaticTask);
            // add 'started' event to PREMIS file!
            try {
                PremisHelper.addTaskEventToPremisFile(automaticTask, null, null, automaticTask.isCorrection());
            } catch (PremisException | ParserConfigurationException e) {
                Helper.setErrorMessage(e.getMessage());
            }
        }
    ...
    private void lockTasksBetweenCurrentAndCorrectionTask(Task currentTask, Task correctionTask) throws DataException {
        ...
        for (Task taskInBetween : allTasksInBetween) {
            ...
            taskService.save(taskInBetween, true);
        }
    }
