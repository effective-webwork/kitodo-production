...
public abstract class SearchService<T extends BaseIndexedBean, S extends BaseDTO, V extends BaseDAO<T>>
    ...
    protected QueryBuilder createSimpleCompareDateQuery(String key, Date date, SearchCondition searchCondition) {
        String dateString = Helper.getDateAsFormattedString(date);
        return createSimpleRangeQuery(key, dateString, searchCondition);
    }

    protected QueryBuilder createSimpleDurationQuery(String key, Duration duration, SearchCondition searchCondition) {
        LocalDateTime creationDate = LocalDateTime.now().minus(duration);
        return createSimpleCompareDateQuery(key, Date.from(creationDate.atZone(ZoneId.systemDefault()).toInstant()),
                searchCondition);
    }

    private QueryBuilder createSimpleRangeQuery(String key, Object object, SearchCondition searchCondition) {
        QueryBuilder query = null;
        switch (searchCondition) {
            case EQUAL:
                query = matchQuery(key, object);
                break;
            case EQUAL_OR_BIGGER:
                query = rangeQuery(key).gte(object);
                break;
            case EQUAL_OR_SMALLER:
                query = rangeQuery(key).lte(object);
                break;
            case BIGGER:
                query = rangeQuery(key).gt(object);
                break;
            case SMALLER:
                query = rangeQuery(key).lt(object);
                break;
            default:
    ...
    protected QueryBuilder createSimpleCompareQuery(String key, Integer value, SearchCondition searchCondition) {
        return createSimpleRangeQuery(key, value, searchCondition);
    }
    ...
    protected SortBuilder getSortBuilder(String sortField, SortOrder sortOrder) {
        if (Objects.nonNull(sortField) && Objects.equals(sortOrder, SortOrder.ASCENDING)) {
            return SortBuilders.fieldSort(sortField).order(org.elasticsearch.search.sort.SortOrder.ASC);
        } else if (Objects.nonNull(sortField) && Objects.equals(sortOrder, SortOrder.DESCENDING)) {
