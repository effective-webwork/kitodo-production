...
public class FileService {
    ...
    /**
     * Convenience function. Create process folder either with or without sub folders.
     *
     * @param process
     *          the process
     * @param withSubfolders
     *          flag indicating whether sub folders should be created as well
     * @return the URI to the process location
     */
    public URI createProcessLocation(Process process, boolean withSubfolders) throws IOException, CommandException {
        if (withSubfolders) {
            return createProcessLocation(process);
        } else {
            return fileManagementModule.createProcessLocation(process.getId().toString());
        }
    }
    ...
    public boolean searchForMedia(Process process, Workpiece workpiece)
        ...
        addNewURIsToExistingPhysicalDivisions(currentMedia,
                workpiece.getAllPhysicalDivisionChildrenSortedFilteredByPageAndTrackAndOther(), canonicals);
    ...
    private List<String> getCanonicalFileNamePartsAndSanitizeAbsoluteURIs(Workpiece workpiece,
        ...
        for (PhysicalDivision physicalDivision : workpiece.getAllPhysicalDivisionChildrenSortedFilteredByPageAndTrackAndOther()) {
    ...
    private void removeMissingMediaFromWorkpiece(List<String> mediaToRemove, Workpiece workpiece,
        ...
        List<PhysicalDivision> pages = workpiece.getAllPhysicalDivisionChildrenSortedFilteredByPageAndTrackAndOther();
        ...
        if (!mediaToRemove.isEmpty()) {
            int i = 1;
            for (PhysicalDivision division : workpiece.getAllPhysicalDivisionChildrenSortedFilteredByPageAndTrackAndOther()) {
                division.setOrder(i);
                i++;
            }
        }
    ...
    private void addNewMediaToWorkpiece(List<String> canonicals, Map<String, Map<Subfolder, URI>> mediaToAdd,
        ...
        for (Entry<String, Map<Subfolder, URI>> entry : mediaToAdd.entrySet()) {
            PhysicalDivision physicalDivision = createPhysicalDivision(entry.getValue(), workpiece);
            View view = new View();
     ...
     */
    private PhysicalDivision createPhysicalDivision(Map<Subfolder, URI> data, Workpiece workpiece) {
        PhysicalDivision physicalDivision = new PhysicalDivision(workpiece);
    ...
    public void renumberPhysicalDivisions(Workpiece workpiece, boolean sortByOrder) {
        ...
        for (PhysicalDivision physicalDivision : sortByOrder
                ? workpiece.getAllPhysicalDivisionChildrenSortedFilteredByPageAndTrackAndOther()
    ...
    private void repaginatePhysicalDivisions(Workpiece workpiece) {
        List<PhysicalDivision> physicalDivisions = workpiece.getAllPhysicalDivisionChildrenSortedFilteredByPageAndTrackAndOther();
        int first = 0;
        String value;
