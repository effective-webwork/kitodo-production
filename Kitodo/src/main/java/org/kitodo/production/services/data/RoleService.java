...
public class RoleService extends ClientSearchDatabaseService<Role, RoleDAO> {
    ...
    /**
     * Detect changes between two given role objects.
     *
     * @param roleBefore Role object before changes
     * @param roleAfter Role object after changes
     * @return HashMap containing changes
     */
    public static HashMap<String, Pair<String, String>> detectChanges(Role roleBefore, Role roleAfter) {
        HashMap<String, Pair<String, String>> changes = new HashMap<>();
        if (!Objects.equals(roleBefore.getTitle(), roleAfter.getTitle())) {
            changes.put("title", Pair.of(roleBefore.getTitle(), roleAfter.getTitle()));
        }
        if (!Objects.equals(roleBefore.getClient(), roleAfter.getClient()) && Objects.nonNull(roleBefore.getClient())) {
            changes.put("client", Pair.of(roleBefore.getClient().getName(), roleAfter.getClient().getName()));
        }
        return changes;
    }
}
