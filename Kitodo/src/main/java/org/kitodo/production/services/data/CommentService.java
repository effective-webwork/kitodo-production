...
public class CommentService extends SearchDatabaseService<Comment, CommentDAO> {
    ...
    /**
     * Return full name of given comments author. If author is NULL, the comment
     * was created by an automatic system process and the method returns "System"
     * as author name accordingly.
     *
     * @param comment Comment for which the user name is returned
     * @return user name of given comments author
     */
    public static String getUsername(Comment comment) {
        if (Objects.nonNull(comment) && Objects.nonNull(comment.getAuthor())) {
