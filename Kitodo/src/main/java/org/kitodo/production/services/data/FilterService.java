...
public class FilterService extends SearchService<Filter, FilterDTO, FilterDAO> {
    ...
    private static int vecteur_project_id = -1;
    ...
    private FilterService() {
        ...
        try (FileInputStream kitodoConfiguration = new FileInputStream(RelayServerServiceConfig.getKitodoPropertiesFile())) {
            ...
            logger.error("Could not load kitodo configuration", e);
        }
    }
    ...
    private QueryBuilder buildQueryFromCondition(String condition, ObjectType objectType) throws DataException {
        ...
        } else if (evaluateFilterString(condition, FilterString.TASKCURRENT, null)) {
            String openFilter = "";
            String inworkFilter = "";
            if (condition.startsWith(FilterString.TASKCURRENT.getFilterEnglish())) {
                openFilter = condition.replaceFirst(FilterString.TASKCURRENT.getFilterEnglish(), FilterString.TASKOPEN.getFilterEnglish());
                inworkFilter = condition.replaceFirst(FilterString.TASKCURRENT.getFilterEnglish(), FilterString.TASKINWORK.getFilterEnglish());
            } else if (condition.startsWith(FilterString.TASKCURRENT.getFilterGerman())) {
                openFilter = condition.replaceFirst(FilterString.TASKCURRENT.getFilterGerman(), FilterString.TASKOPEN.getFilterGerman());
                inworkFilter = condition.replaceFirst(FilterString.TASKCURRENT.getFilterGerman(), FilterString.TASKINWORK.getFilterGerman());
            }
            return new BoolQueryBuilder().minimumShouldMatch(1)
                    .should(createTaskFilters(openFilter, FilterString.TASKOPEN, TaskStatus.OPEN, false, objectType))
                    .should(createTaskFilters(inworkFilter, FilterString.TASKINWORK, TaskStatus.INWORK, false, objectType));
        ...
        } else if (evaluateFilterString(condition, FilterString.DURATION, null)) {
            return createProcessDurationFilter(condition, objectType);
    ...
    private QueryBuilder createProcessDurationFilter(String filter, ObjectType objectType) throws DataException {
        SearchCondition searchCondition = SearchCondition.EQUAL;
        String searchTerm = "";
        if (StringUtils.countMatches(filter, ">") == 1 && StringUtils.countMatches(filter, "<") == 0) {
            searchCondition = SearchCondition.SMALLER;
            searchTerm = filter.substring(filter.indexOf(">") + 1);
        } else if (StringUtils.countMatches(filter, "<") == 1 && StringUtils.countMatches(filter, ">") == 0) {
            searchCondition = SearchCondition.BIGGER;
            searchTerm = filter.substring(filter.indexOf("<") + 1);
        }
        QueryBuilder durationQuery = createSimpleDurationQuery(ProcessTypeField.CREATION_DATE.getKey(), Duration.parse(searchTerm),
                searchCondition);
        if (objectType == ObjectType.PROCESS) {
            return durationQuery;
        } else if (objectType == ObjectType.TASK) {
            return getQueryAccordingToObjectTypeAndSearchInObject(ObjectType.TASK, ObjectType.PROCESS, durationQuery);
        }
        return new BoolQueryBuilder();
    }
    ...
    public List<String> initStepTitles() {
        ...
        try {
            // Vecteur: Only show tasks from current template in extended search/filter suggestions.
            if (vecteur_project_id != -1) {
                taskTitles = ServiceManager.getProjectService().getById(vecteur_project_id).getTemplates().get(0).getTasks().stream()
                        .sorted(Comparator.comparing(Task::getOrdering).thenComparing(Task::getTitle))
                        .map(Task::getTitle)
                        .collect(Collectors.toList());
                return taskTitles;
            }
