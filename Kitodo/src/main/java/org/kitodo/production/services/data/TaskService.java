...
public class TaskService extends ProjectSearchService<Task, TaskDTO, TaskDAO> {
    ...
    private static final String DATENIMPORT = "Datenimport";
    ...
    private BoolQueryBuilder createUserTaskQuery(String filter, boolean onlyOwnTasks, boolean hideCorrectionTasks,
                                                 ...
                                                 ProcessPriority processPriorityRestriction) {
        ...
        if (Objects.nonNull(processPriorityRestriction)) {
            query.must(getQueryForProcessPriority(processPriorityRestriction.getValue()));
        }
        ...
        if (hideCorrectionTasks || !ServiceManager.getSecurityAccessService().hasAuthorityToViewCorrectionTasks()) {
    ...
    public Long countResults(Map filters) throws DataException {
        return countResults(new HashMap<String, String>(filters), false, false, false, Collections.emptyList(), null);
    }

    public Long countResults(HashMap<String, String> filters, boolean onlyOwnTasks, boolean hideCorrectionTasks,
                             boolean showAutomaticTasks, List<TaskStatus> taskStatus, ProcessPriority processPriority)
            throws DataException {
        return countDocuments(createUserTaskQuery(ServiceManager.getFilterService().parseFilterString(filters),
                onlyOwnTasks, hideCorrectionTasks, showAutomaticTasks, taskStatus, processPriority));
    }
    ...
    public List<TaskDTO> loadData(int first, int pageSize, String sortField, SortOrder sortOrder, Map filters)
        ...
        return loadData(first, pageSize, sortField, sortOrder, filters, false, false, false,
                Arrays.asList(TaskStatus.OPEN, TaskStatus.INWORK), null);
    }
     ...
     * @param processPriority as ProcessPriority
    ...
    public List<TaskDTO> loadData(int first, int pageSize, String sortField, SortOrder sortOrder, Map filters,
                                  ...
                                  List<TaskStatus> taskStatus, ProcessPriority processPriority)
            throws DataException {
        ...
        return findByQuery(createUserTaskQuery(filter, onlyOwnTasks, hideCorrectionTasks, showAutomaticTasks,
                taskStatus, processPriority), getSortBuilder(sortField, sortOrder), first, pageSize, false);
    }
    ...
    /**
     * Find distinct task titles of open processes.
     *
     * @return a list of distinct task titles of open processes.
     */
    public List<String> findTaskTitlesDistinctOfOpenProcesses() throws DataException {
        BoolQueryBuilder query = new BoolQueryBuilder();
        query.mustNot(ServiceManager.getProcessService().getQueryForClosedProcesses());
        List<ProcessDTO> openProcesses = ServiceManager.getProcessService().findByQuery(query, false);
        List<TaskDTO> tasks = openProcesses.stream().flatMap(p -> p.getTasks().stream()).collect(Collectors.toList());
        List<TaskDTO> manualTasks = tasks.stream().filter(t -> !t.isTypeAutomatic()).collect(Collectors.toList());
        List<TaskDTO> automaticTasks = tasks.stream().filter(TaskDTO::isTypeAutomatic).collect(Collectors.toList());
        // manual standard tasks
        List<String> taskTitles = manualTasks.stream()
                .filter(t -> !t.getTitle().contains(EXTERN) && !t.getTitle().contains(DATENIMPORT))
                .sorted(Comparator.comparing(TaskDTO::getOrdering).thenComparing(TaskDTO::getTitle))
                .map(TaskDTO::getTitle).distinct().collect(Collectors.toList());
        // manual external tasks
        taskTitles.addAll(manualTasks.stream()
                .filter(t -> t.getTitle().contains(EXTERN) || t.getTitle().contains(DATENIMPORT))
                .sorted(Comparator.comparing(TaskDTO::getOrdering))
                .map(TaskDTO::getTitle).distinct().collect(Collectors.toList()));
        // automatic tasks
        taskTitles.addAll(automaticTasks.stream()
                .sorted(Comparator.comparing(TaskDTO::getOrdering))
                .map(TaskDTO::getTitle).distinct().collect(Collectors.toList()));
        return taskTitles;
    }
    ...
    public boolean executeScript(Task task, String script, boolean automatic) throws DataException {
        ...
        try {
            ...
            task = ServiceManager.getTaskService().getById(task.getId());
    ...
    private void finishOrReturnAutomaticTask(Task task, boolean automatic, boolean successful)
        ...
        if (automatic) {
            ...
            if (successful) {
                ...
                for (Task processTask : task.getProcess().getTasks()) {
                    if (processTask.getOrdering() < task.getOrdering()
                            && (processTask.getProcessingStatus().equals(TaskStatus.OPEN)
                            || processTask.getProcessingStatus().equals(TaskStatus.INWORK))) {
     ...
     * @return query as QueryBuilder
     */
    private QueryBuilder getQueryForProcessPriority(int processPriority) {
        return createSimpleQuery(TaskTypeField.PROCESS_PRIORITY.getKey(), processPriority, true);
    }
    ...
    private static Stream<Folder> removeFoldersThatCannotBeGenerated(Stream<Folder> folders) {
        return folders.filter(folder -> folder.getDerivative().isPresent() || folder.getDpi().isPresent()
                || folder.getImageSize().isPresent() || folder.isCustom());
    }
    ...
    public static int getCorrespondingTemplateTaskId(Task task) {
        if (Objects.nonNull(task) && Objects.nonNull(task.getProcess()) && Objects.nonNull(task.getProcess().getTemplate())) {
