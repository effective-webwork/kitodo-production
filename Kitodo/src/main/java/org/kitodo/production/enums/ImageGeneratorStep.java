...
public enum ImageGeneratorStep implements Consumer<ImageGenerator> {
    ...
    DETERMINE_WHICH_IMAGES_NEED_TO_BE_GENERATED {
        ...
        public void accept(ImageGenerator imageGenerator) {
            ...
            List<Subfolder> subfoldersWhoseContentsAreToBeGenerated = imageGenerator
                    .determineFoldersThatNeedDerivatives(source);
    ...
    GENERATE_IMAGES {
        @Override
        public void accept(ImageGenerator imageGenerator) {
            ContentToBeGenerated instruction = imageGenerator.getFromContentToBeGeneratedByPosition();
            imageGenerator.letTheSupervisorDo(emptyTask -> emptyTask.setWorkDetail(
                Helper.getTranslation("generateImages", instruction.getCanonical())));
            LogManager.getLogger(ImageGeneratorStep.class).info("Generating ".concat(instruction.toString()));
            imageGenerator.createDerivatives(instruction);
