...
public class DesktopForm extends BaseForm {
    ...
    private static final String SORT_CREATION_DATE = "creationDate";
    ...
    public List<ProcessDTO> getProcesses() {
        try {
            if (ServiceManager.getSecurityAccessService().hasAuthorityToViewProcessList() && processList.isEmpty()) {
                processList = ServiceManager.getProcessService().loadData(0, 10, SORT_CREATION_DATE, SortOrder.ASCENDING, null);
            }
        } catch (DataException | JsonException e) {
    ...
    public long getNumberOfElements(ObjectType objectType) {
        try {
            switch (objectType) {
                case TASK:
                    return ServiceManager.getTaskService().countResults(Collections.emptyMap());
                case USER:
                    return ServiceManager.getUserService().countResults(Collections.emptyMap());
                case DOCKET:
                    return ServiceManager.getDocketService().countResults(Collections.emptyMap());
                case PROCESS:
                    return ServiceManager.getProcessService().countResults(Collections.emptyMap());
                case PROJECT:
                    return ServiceManager.getProjectService().countResults(Collections.emptyMap());
                case RULESET:
                    return ServiceManager.getRulesetService().countResults(Collections.emptyMap());
                case TEMPLATE:
                    return ServiceManager.getTemplateService().countResults(Collections.emptyMap());
                case ROLE:
                    return ServiceManager.getRoleService().countResults(Collections.emptyMap());
                case WORKFLOW:
                    return ServiceManager.getWorkflowService().countResults(Collections.emptyMap());
                default:
                    return 0L;
            }

        } catch (DAOException | JsonException | ElasticsearchStatusException | DataException e) {
