...
public class ProjectForm extends BaseForm {
    ...
    private Project originalStateProject;
    private List<Folder> originalFolders;
    private List<Template> originalTemplates;
    private String originalPreviewPath;
    private String originalMediaViewPath;
    private String originalGeneratorSourcePath;
    ...
    public String save() {
        ...
        if (isTitleValid()) {
            try {
                ...
                logChanges(this.originalStateProject, this.project);
    ...
    private void commitTemplates() throws DAOException {
        ...
        // TODO: log "save" message only for _new_ projects!
        logger.info(AuditingLogger.createObjectSaveOrDeleteLogEntry(LogEntryType.SAVE,
                ObjectType.PROJECT.getMessageKeySingular(), this.project.getTitle(), this.project.getId()));
    ...
    public void delete(int projectId) {
        try {
            String projectTitle = ServiceManager.getProjectService().findById(projectId).getTitle();
            ProjectService.delete(projectId);
            logger.info(AuditingLogger.createObjectSaveOrDeleteLogEntry(LogEntryType.DELETE,
                    ObjectType.PROJECT.getMessageKeySingular(), projectTitle, projectId));
        } catch (DAOException | DataException e) {
    ...
    /**
     * Store original project state for change log computation on saving.
     */
    private void storeOriginalState() {
        this.originalStateProject = ServiceManager.getProjectService().duplicateProject(this.project);
        this.originalStateProject.setTitle(this.project.getTitle());
        this.originalStateProject.setId(this.project.getId());
        this.originalFolders = new LinkedList<>(this.project.getFolders());
        this.originalTemplates = new LinkedList<>(this.project.getTemplates());
        if (Objects.nonNull(this.project.getPreview())) {
            this.originalPreviewPath = this.project.getPreview().getRelativePath();
        }
        if (Objects.nonNull(this.project.getMediaView())) {
            this.originalMediaViewPath = this.project.getMediaView().getRelativePath();
        }
        if (Objects.nonNull(this.project.getGeneratorSource())) {
            this.originalGeneratorSourcePath = this.project.getGeneratorSource().getRelativePath();
        }
    }

    private void logChanges(Project oldProject, Project newProject) {
        if (Objects.nonNull(oldProject)) {
            HashMap<String, Pair<String, String>> changes = ProjectService.detectChanges(oldProject, newProject);
            for (Map.Entry<String, Pair<String, String>> change : changes.entrySet()) {
                logger.info(AuditingLogger.createFieldUpdateLogEntry(change.getKey(), change.getValue().getLeft(),
                        change.getValue().getRight(), ObjectType.PROJECT.getMessageKeySingular(), oldProject.getTitle(),
                        oldProject.getId()));
            }
        }
        if (Objects.nonNull(this.originalTemplates)) {
            logTemplateChanges(newProject);
        }
        if (Objects.nonNull(this.originalFolders)) {
            logFolderChanges(newProject);
        }
    }

    private void logTemplateChanges(Project newProject) {

        HashSet<Template> removedTemplates = new HashSet<>(this.originalTemplates);
        removedTemplates.removeAll(new HashSet<>(newProject.getTemplates()));
        for (Template template : removedTemplates) {
            logger.info(AuditingLogger.createPropertyAddOrRemoveLogEntry(LogEntryType.REMOVE,
                    ObjectType.TEMPLATE.getMessageKeySingular(), template.getTitle(), template.getId(),
                    ObjectType.PROJECT.getMessageKeySingular(), this.project.getTitle(), this.project.getId()));
        }

        HashSet<Template> addedTemplates = new HashSet<>(newProject.getTemplates());
        addedTemplates.removeAll(new HashSet<>(this.originalTemplates));
        for (Template template : addedTemplates) {
            logger.info(AuditingLogger.createPropertyAddOrRemoveLogEntry(LogEntryType.ADD,
                    ObjectType.TEMPLATE.getMessageKeySingular(), template.getTitle(), template.getId(),
                    ObjectType.PROJECT.getMessageKeySingular(), this.project.getTitle(), this.project.getId()));
        }
    }

    private void logFolderChanges(Project newProject) {
        if (Objects.nonNull(newProject.getPreview()) && Objects.nonNull(this.originalPreviewPath)
                && !Objects.equals(newProject.getPreview().getRelativePath(), this.originalPreviewPath)) {
            logger.info(AuditingLogger.createFieldUpdateLogEntry("previewFolderPath", this.originalPreviewPath,
                    newProject.getPreview().getRelativePath(), ObjectType.PROJECT.getMessageKeySingular(),
                    newProject.getTitle(), newProject.getId()));
        }

        if (Objects.nonNull(newProject.getMediaView()) && Objects.nonNull(this.originalMediaViewPath)
                && !Objects.equals(newProject.getMediaView().getRelativePath(), this.originalMediaViewPath)) {
            logger.info(AuditingLogger.createFieldUpdateLogEntry("mediaViewFolderPath", this.originalMediaViewPath,
                    newProject.getMediaView().getRelativePath(), ObjectType.PROJECT.getMessageKeySingular(),
                    newProject.getTitle(), newProject.getId()));
        }

        if (Objects.nonNull(newProject.getGeneratorSource()) && Objects.nonNull(this.originalGeneratorSourcePath)
                && !Objects.equals(newProject.getGeneratorSource().getRelativePath(), this.originalGeneratorSourcePath)) {
            logger.info(AuditingLogger.createFieldUpdateLogEntry("generatorSourceFolderPath", this.originalGeneratorSourcePath,
                    newProject.getGeneratorSource().getRelativePath(), ObjectType.PROJECT.getMessageKeySingular(),
                    newProject.getTitle(), newProject.getId()));
        }

        HashSet<Folder> removedFolders = new HashSet<>(this.originalFolders);
        removedFolders.removeAll(new HashSet<>(newProject.getFolders()));
        for (Folder folder : removedFolders) {
            logger.info(AuditingLogger.createPropertyAddOrRemoveLogEntry(LogEntryType.REMOVE,
                    ObjectType.FOLDER.getMessageKeySingular(), folder.getRelativePath(), folder.getId(),
                    ObjectType.PROJECT.getMessageKeySingular(), this.project.getTitle(), this.project.getId()));
        }

        HashSet<Folder> addedFolders = new HashSet<>(newProject.getFolders());
        addedFolders.removeAll(this.originalFolders);
