...
public class TemplateForm extends TemplateBaseForm {
    ...
    private Template originalStateTemplate;
    private List<Project> originallyAssignedProjects;
    ...
    public String save() {
        if (isTitleValid()) {
            ...
            try {
                ...
                logChanges(this.originalStateTemplate, this.template);
        ...
        if (!this.template.getProcesses().isEmpty()) {
            ...
            try {
                ...
                logger.info(AuditingLogger.createObjectSaveOrDeleteLogEntry(LogEntryType.DELETE,
                        ObjectType.TEMPLATE.getMessageKeySingular(), this.template.getTitle(), this.template.getId()));
            } catch (Exception e) {
    ...
    private void logChanges(Template oldTemplate, Template newTemplate) {
        if (Objects.nonNull(oldTemplate)) {
            HashMap<String, Pair<String, String>> changes = TemplateService.detectChanges(oldTemplate, newTemplate);
            for (Map.Entry<String, Pair<String, String>> change : changes.entrySet()) {
                logger.info(AuditingLogger.createFieldUpdateLogEntry(change.getKey(), change.getValue().getLeft(),
                        change.getValue().getRight(), ObjectType.TEMPLATE.getMessageKeySingular(), oldTemplate.getTitle(),
                        oldTemplate.getId()));
            }
        }
        if (Objects.nonNull(this.originallyAssignedProjects)) {
            logProjectChanges(newTemplate);
        }
    }

    private void logProjectChanges(Template newTemplate) {
        HashSet<Project> removedProjects = new HashSet<>(this.originallyAssignedProjects);
        removedProjects.removeAll(new HashSet<>(newTemplate.getProjects()));

        for (Project project : removedProjects) {
            logger.info(AuditingLogger.createPropertyAddOrRemoveLogEntry(LogEntryType.REMOVE,
                    ObjectType.PROJECT.getMessageKeySingular(), project.getTitle(), project.getId(),
                    ObjectType.TEMPLATE.getMessageKeySingular(), newTemplate.getTitle(), newTemplate.getId()));
        }

        HashSet<Project> addedProjects = new HashSet<>(newTemplate.getProjects());
        addedProjects.removeAll(this.originallyAssignedProjects);
    ...
    private void storeOriginalState() {
        this.originalStateTemplate = ServiceManager.getTemplateService().duplicateTemplate(this.template);
        this.originalStateTemplate.setTitle(this.template.getTitle());
        this.originalStateTemplate.setId(this.template.getId());
        this.originallyAssignedProjects = new LinkedList<>(this.assignedProjects);
    }

    private void resetOriginalState() {
        this.originalStateTemplate = null;
        this.originallyAssignedProjects = null;
    }
}
