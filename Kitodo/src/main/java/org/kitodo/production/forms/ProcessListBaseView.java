...
public class ProcessListBaseView extends BaseForm {
        ...
        try {
            ...
            if (process.getChildren().isEmpty()) {
                try {
                    ...
                    logger.info(AuditingLogger.createProcessManipulationLogEntry(LogEntryType.PROCESS_DELETE, process));
    ...
    /**
     * Check whether process has a Viaduc status that allows him to be cancelled.
     *
     * @param processId ID of the process for which the Viaduc state is checked
     * @return whether the process can be cancelled or not
     */
    public boolean canBeCanceled(int processId) {
        Process cancelProgress;
        try {
            cancelProgress = ServiceManager.getProcessService().getById(processId);
        } catch (DAOException e) {
            logger.error("Unable to retrieve process with ID " + processId);
            return false;
        }
        String viaducState = JobService.getViaducState(cancelProgress);
        return Objects.nonNull(viaducState)
                && (viaducState.equals("READY_FOR_DIGITIZATION") || viaducState.equals("IN_PROGRESS") || viaducState.equals("EXTERNAL"));
    }
