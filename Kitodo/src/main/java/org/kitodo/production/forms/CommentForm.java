...
public class CommentForm extends BaseForm {
    ...
    public String addComment() {
        ...
        if (comment.getType().equals(CommentType.ERROR)) {
            ...
            try {
                PremisHelper.addGeneralCommentEventToPremisFile(comment);
    ...
    private void logComment(Comment comment) {
        String processLog = "process (Viaduc ID: "
                + JobService.getViaducId(comment.getProcess()) + "; Kitodo ID: " + comment.getProcess().getId()
                + ") by user " + CommentService.getUsername(comment) + ".";
        if (CommentType.INFO.equals(comment.getType())) {
            logger.info("Comment '" + comment.getMessage() + "' added to " + processLog);
        } else if (CommentType.ERROR.equals(comment.getType()) && Objects.nonNull(comment.getCurrentTask())) {
            logger.info("Correction comment '" + comment.getMessage() + "' added to task '"
                    + comment.getCurrentTask().getTitle() + "' of " + processLog);
        }
    }
    ...
    public boolean isStructuralError() {
        return this.process.isStructuralError();
    }

    /**
     * Set structuralError.
     *
     * @param structuralError as boolean
     */
    public void setStructuralError(boolean structuralError) {
        this.process.setStructuralError(structuralError);
        this.saveProcess();
    }
    ...
    public boolean isMissingInformation() {
        return process.isMissingInformation();
    }

    /**
     * Set missingInformation.
     *
     * @param missingInformation as boolean
     */
    public void setMissingInformation(boolean missingInformation) {
        this.process.setMissingInformation(missingInformation);
        this.saveProcess();
    }
    ...
    public boolean isReimport() {
        return process.isReimport();
    }

    /**
     * Set reimport.
     *
     * @param reimport as boolean
     */
    public void setReimport(boolean reimport) {
        this.process.setReimport(reimport);
        this.saveProcess();
    }
    ...
    private void saveProcess() {
        try {
            ...
            Helper.setErrorMessage("Unable to save reporting flags to process: " + e.getMessage());
        }
    }
