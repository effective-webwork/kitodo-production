...
public class AddPhysicalDivisionDialog {
    ...
    public void addPhysicalDivision() {
        ...
        if (selectedPhysicalDivision.isPresent()) {
            ...
            if (Objects.nonNull(physicalDivision)) {
                if ("partialJob".equals(selectedType)) {
                    physicalDivision.setOrderlabel(String.format("%03d", dataEditor.getMaxPartialJobId() + 1));
                    logger.info(AuditingLogger.createStructuralElementAddedOrRemovedLogEntry(
                            LogEntryType.METADATA_EDITOR_STRUCTURAL_ELEMENT_ADDED, "partial job",
                ...
                Helper.setErrorMessage("Unable to add physical division!");
            }
        } else {
    ...
    public InsertionPosition getSelectedPosition() {
        if (Objects.isNull(selectedPosition)) {
            selectedPosition = InsertionPosition.LAST_CHILD_OF_CURRENT_ELEMENT;
        }
