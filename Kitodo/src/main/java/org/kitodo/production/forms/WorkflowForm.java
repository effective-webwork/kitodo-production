...
public class WorkflowForm extends BaseForm {
    ...
    private String originalWorkflowStatus;
    ...
    public String saveAndRedirect() {
        ...
        try {
            if (saveFiles()) {
                ...
                logger.info("Workflow " + this.workflow.getId() + " saved by user "
        ...
        if (!this.workflow.getTemplates().isEmpty()) {
            ...
            try {
                ...
                logger.info(AuditingLogger.createObjectSaveOrDeleteLogEntry(LogEntryType.DELETE,
                        ObjectType.WORKFLOW.getMessageKeySingular(), this.workflow.getTitle(), this.workflow.getId()));
    ...
    public void setWorkflow(Workflow workflow) {
        ...
        this.originalWorkflowTitle = this.workflow.getTitle();
        this.originalWorkflowStatus = this.workflow.getStatus().toString();
    }
    ...
    private void logChanges() {
        if (!Objects.equals(this.originalWorkflowTitle, this.workflow.getTitle())) {
            logger.info(AuditingLogger.createFieldUpdateLogEntry("title", this.originalWorkflowTitle,
                    this.workflow.getTitle(), ObjectType.WORKFLOW.getMessageKeySingular(), this.workflow.getTitle(),
                    this.workflow.getId()));
        }
        if (!Objects.equals(this.originalWorkflowStatus, this.workflow.getStatus().toString())) {
            logger.info(AuditingLogger.createFieldUpdateLogEntry("status", this.originalWorkflowStatus,
                    this.workflow.getStatus().toString(), ObjectType.WORKFLOW.getMessageKeySingular(),
                    this.workflow.getTitle(), this.workflow.getId()));
        }
    }
