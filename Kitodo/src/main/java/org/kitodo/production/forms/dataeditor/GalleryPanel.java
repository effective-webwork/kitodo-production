...
public class GalleryPanel {
    ...
    void updateSelection(PhysicalDivision physicalDivision, LogicalDivision structuralElement) {
        if (!physicalDivision.getMediaFiles().isEmpty()) {
            ...
            dataEditor.getSelectedMedia().clear();
            if (Objects.equals(physicalDivision.getType(), PARTIAL_JOB)) {
                dataEditor.getSelectedMedia().add(new ImmutablePair<>(physicalDivision, null));
            }
        }
    }
    ...
    void show() {
        ...
        List<PhysicalDivision> physicalDivisions = dataEditor.getWorkpiece()
                .getAllPhysicalDivisionChildrenSortedFilteredByPageAndTrackAndOther();
        ...
        for (GalleryStripe galleryStripe : stripes) {
            for (GalleryMediaContent mediaContent : galleryStripe.getMedias()) {
                if (Objects.nonNull(galleryMediaContent.getId())
                        && Objects.nonNull(mediaContent.getId())
                        && galleryMediaContent.getId().equals(mediaContent.getId())) {
