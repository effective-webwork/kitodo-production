...
public class SearchForm {
    ...
    private String createFilter() {
        ...
        if (!this.idin.isEmpty()) {
            search += "\"" + FilterString.ID.getFilterString() + this.idin + "\" ";
        }
        if (!this.processParentId.isEmpty()) {
            search += "\"" + FilterString.PARENTPROCESSID.getFilterString() + this.processParentId + "\" ";
        }
        if (!this.project.isEmpty()) {
            search += "\"" + this.projectOperand + FilterString.PROJECT.getFilterString() + this.project + "\" ";
        }
        ...
        if (!this.stepdonetitle.isEmpty() && !this.stepdoneuser.isEmpty()
            ...
            search += "\"" + FilterString.TASKDONEUSER.getFilterString() + this.stepdoneuser + "\" \""
                    + FilterString.TASKDONETITLE.getFilterString() + this.stepdonetitle + "\" ";
        }
    ...
    private String createSearchProperty(String title, String value, String operand, FilterString filterString) {
        if (Objects.nonNull(value) && !value.isEmpty()) {
            if (Objects.nonNull(title) && !title.isEmpty()) {
                return "\"" + operand + filterString.getFilterString() + title + ":" + value + "\" ";
            } else {
                return "\"" + operand + filterString.getFilterString() + value + "\" ";
            }
        }
        return "";
    }
    ...
    public String getTaskCurrentString() {
        return FilterString.TASKCURRENT.getFilterString();
    }

    public static String getFilterString(String filter) {
        for (FilterString filterString : FilterString.values()) {
            if (filterString.getFilterGerman().startsWith(filter)
                    || filterString.getFilterEnglish().startsWith(filter)) {
                return filterString.getFilterString();
            }
        }
        return "";
    }
