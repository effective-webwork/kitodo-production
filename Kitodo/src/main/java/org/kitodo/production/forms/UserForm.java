...
public class UserForm extends BaseForm {
    ...
    private User originalStateUser;
    ...
    public String newUser() {
        ...
        this.userObject.setShortcuts(ConfigCore.getParameterOrDefaultValue(ParameterCore.DEFAULT_USER_SHORTCUTS));
        return userEditPath;
    }
    ...
    void deleteUser(User user) {
        try {
            ...
            logger.info(AuditingLogger.createObjectSaveOrDeleteLogEntry(LogEntryType.DELETE,
                    ObjectType.USER.getMessageKeySingular(), user.getFullName(), user.getId()));
        } catch (DAOException e) {
            Helper.setErrorMessage(ERROR_SAVING, new Object[]{ObjectType.USER.getTranslationSingular()}, logger, e);
        }
    }
            ...
            try {
                ...
                for (Role role : this.userObject.getRoles()) {
                    if (role.getId().equals(roleId)) {
                        ...
                        loggedChanges.add(createPropertyAddOrRemoveLogEntry("role", role.getTitle(), role.getId(),
                                LogEntryType.REMOVE));
                        break;
                    }
                }
            ...
            try {
                ...
                for (Client client : this.userObject.getClients()) {
                    if (client.getId().equals(clientId)) {
                        ...
                        loggedChanges.add(createPropertyAddOrRemoveLogEntry("client", client.getName(), client.getId(),
                                LogEntryType.REMOVE));
                        break;
                    }
                }
            ...
            try {
                ...
                for (Project project : this.userObject.getProjects()) {
                    if (project.getId().equals(projectId)) {
                        ...
                        loggedChanges.add(createPropertyAddOrRemoveLogEntry("project", project.getTitle(),
                                project.getId(), LogEntryType.REMOVE));
                        break;
                    }
                }
            } catch (NumberFormatException e) {
        ...
        if (Objects.nonNull(idParameter)) {
            ...
            try {
                ...
                if (!this.userObject.getProjects().contains(project)) {
                    ...
                    loggedChanges.add(createPropertyAddOrRemoveLogEntry("project", project.getTitle(), project.getId(),
                            LogEntryType.ADD));
                }
            } catch (DAOException e) {
                Helper.setErrorMessage(ERROR_DATABASE_READING,
    ...
    public void load(int id) {
        ...
        try {
            ...
            resetLoggedChanges();
            this.originalStateUser = new User(this.userObject);
        } catch (DAOException e) {
    ...
    private String createPropertyAddOrRemoveLogEntry(String propertyType, String propertyName, Integer propertyId,
                                                     LogEntryType type) {
        return AuditingLogger.createPropertyAddOrRemoveLogEntry(type, propertyType, propertyName, propertyId,
                ObjectType.USER.getMessageKeySingular(), this.userObject.getFullName(), this.userObject.getId());
    }
    ...
    private void logChanges(User oldUser, User newUser) {
        HashMap<String, Pair<String, String>> changes = UserService.detectChanges(oldUser, newUser);
        ...
        for (String loggedChange : this.loggedChanges) {
            logger.info(loggedChange);
        }
    }
