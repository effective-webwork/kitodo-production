...
public class LockingConflictsException extends IOException {
    ...
    public String getLocalizedMessage() {
        ...
        if (numberOfConflicts == 1) {
            String conflictingUri = conflicts.entrySet().iterator().next().getKey().toString();
            return Helper.getTranslation("dataEditor.lockingConflict", conflictingUri, conflictingUsers());
        } else {
            return Helper.getTranslation("dataEditor.lockingConflicts", Integer.toString(numberOfConflicts),
                    conflictingUsers());
        }
    }
}
