...
public class ProcessForm extends TemplateBaseForm {
    ...
    private HashMap<String, List<HashMap<String, Object>>> taskProcessingTimes = new HashMap<>();
    ...
    public static String getTimeSinceRetrieved(ProcessDTO processDTO) {
        return ProcessService.getTimeSinceRetrieved(processDTO);
    }

    public static String getDateRetrieved(ProcessDTO processDTO) {
        return ProcessService.getDateRetrieved(processDTO);
    }

    public static String getDateRequested(ProcessDTO processDTO) {
        return ProcessService.getDateRequested(processDTO);
    }
    ...
    public String save() {
        if (Objects.nonNull(process) && Objects.nonNull(newProcessTitle)) {
            ...
            try {
                ...
                logger.info(AuditingLogger.createProcessManipulationLogEntry(LogEntryType.PROCESS_SAVE, this.process));
    ...
    /**
     * Export process and generate SIP package.
     * @param id int containing the process id
     */
    public void exportSip(int id) {
        try {
            this.process = ServiceManager.getProcessService().getById(id);
            SIPHandler sipHandler = new SIPHandler(this.process);
            sipHandler.generateSIP();
        } catch (DAOException e) {
            e.printStackTrace();
        }
    }
    ...
    public void setTaskStatusUp() throws DataException, IOException, DAOException {
        String previousTaskState = this.task.getProcessingStatus().getTitle();
        workflowControllerService.setTaskStatusUp(this.task);
        ProcessService.deleteSymlinksFromUserHomes(this.task);
        refreshParent();
        logger.info(AuditingLogger.createTaskStateChangedLogEntry(task, previousTaskState));
    }
     ...
     */
    public String getExternalStatus(int processID) {
        try {
            Process process = ServiceManager.getProcessService().getById(processID);
            return JobService.getExternalStatus(process);
    ...
    public List<List<String>> getProcessContainers(ProcessDTO processDTO) {
        return ServiceManager.getProcessService().getProcessContainers(processDTO);
    }
    ...
    public String formatProcessContainers(List<List<String>> containerList) {
        return ServiceManager.getProcessService().formatProcessContainers(containerList);
    }

    /**
     * Get priorityRestriction.
     *
     * @return value of priorityRestriction
     */
    public ProcessPriority getPriorityRestriction() {
        return ((LazyProcessDTOModel)this.lazyDTOModel).getPriorityRestriction();
    }

    /**
     * Set priorityRestriction.
     *
     * @param priorityRestriction as org.kitodo.data.database.enums.ProcessPriority
     */
    public void setPriorityRestriction(ProcessPriority priorityRestriction) {
        ((LazyProcessDTOModel)this.lazyDTOModel).setPriorityRestriction(priorityRestriction);
    }

    /**
     * Loads task information from PREMIS file.
     */
    private void loadPremisEvents() {
        try {
            taskProcessingTimes = PremisHelper.getTaskEventData(this.process);
        } catch (ParserConfigurationException | IOException | PremisException e) {
            Helper.setErrorMessage(e);
        }
    }

    public String getLastUsername(String task) {
        if (taskProcessingTimes.containsKey(task)
                && !taskProcessingTimes.get(task).isEmpty()) {
            HashMap<String, Object> lastEntry = taskProcessingTimes.get(task).get(taskProcessingTimes.get(task).size() - 1);
            if (Objects.nonNull(lastEntry.get(XMLHelper.USER))) {
                return (String) lastEntry.get(XMLHelper.USER);
            }
        }
        return "-";
    }

    public Date getLastStartedTimestamp(String task) {
        if (taskProcessingTimes.containsKey(task)
                && !taskProcessingTimes.get(task).isEmpty()) {
            HashMap<String, Object> lastEntry = taskProcessingTimes.get(task)
                    .get(taskProcessingTimes.get(task).size() - 1);
            if (lastEntry.get(XMLHelper.STARTED) instanceof Date) {
                return (Date)lastEntry.get(XMLHelper.STARTED);
            }
        }
        return null;
    }

    public Date getLastFinishedTimestamp(String task) {
        if (taskProcessingTimes.containsKey(task)
                && !taskProcessingTimes.get(task).isEmpty()) {
            HashMap<String, Object> lastEntry = taskProcessingTimes.get(task)
                    .get(taskProcessingTimes.get(task).size() - 1);
            if (lastEntry.get(XMLHelper.FINISHED) instanceof Date) {
                return (Date)lastEntry.get(XMLHelper.FINISHED);
            }
        }
        return null;
    }

    public List<HashMap<String, Object>> getPreviousTaskEvents(String taskTitle) {
        if (taskProcessingTimes.containsKey(taskTitle) && taskProcessingTimes.get(taskTitle).size() > 1) {
            List<HashMap<String, Object>> previousTaskEvents = new ArrayList<>(taskProcessingTimes.get(taskTitle)
                    .subList(0, taskProcessingTimes.get(taskTitle).size() - 1));
            Collections.reverse(previousTaskEvents);
            return previousTaskEvents;
        } else {
    ...
    public static String getUserString() {
        return XMLHelper.USER;
    }

    public static String getStartedString() {
        return XMLHelper.STARTED;
    }

    public static String getFinishedString() {
        return XMLHelper.FINISHED;
    }

    // TODO: call "JobService.storeJobReport" in cleanup method instead!
    public static void createReport(int processId) {
        try {
            Process process = ServiceManager.getProcessService().getById(processId);
