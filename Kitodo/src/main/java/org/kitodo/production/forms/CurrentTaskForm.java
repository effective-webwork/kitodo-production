...
public class CurrentTaskForm extends BaseForm {
        ...
        if (this.currentTask.getProcessingStatus() != TaskStatus.OPEN) {
            ...
            try {
                if (this.currentTask.isTypeAcceptClose()) {
                    ...
                    logger.info(AuditingLogger.createProcessTaskAssignedLogEntry(LogEntryType.TASK_ASSIGNED, this.currentTask));
                }
    ...
    public String releaseTask() {
        try {
            ...
            logger.info(AuditingLogger.createProcessTaskAssignedLogEntry(LogEntryType.TASK_UNASSIGNED, this.currentTask));
        } catch (DataException e) {
            Helper.setErrorMessage(ERROR_SAVING, new Object[] {ObjectType.TASK.getTranslationSingular() }, logger, e);
            return this.stayOnCurrentPage;
        }
    ...
    /**
     * Get processPriorityRestriction.
     *
     * @return value of processPriorityRestriction
     */
    public ProcessPriority getProcessPriorityRestriction() {
        return ((LazyTaskDTOModel)this.lazyDTOModel).getProcessPriorityRestriction();
    }

    /**
     * Set processPriorityRestriction.
     *
     * @param processPriorityRestriction as org.kitodo.data.database.enums.ProcessPriority
     */
    public void setProcessPriorityRestriction(ProcessPriority processPriorityRestriction) {
        ((LazyTaskDTOModel) this.lazyDTOModel).setProcessPriorityRestriction(processPriorityRestriction);
    }
     ...
     * @return id of the template task or -1 if no matching task could be found
     */
    public static int getCorrespondingTemplateTaskIdFromDTO(TaskDTO task) {
        try {
            return TaskService.getCorrespondingTemplateTaskId(ServiceManager.getTaskService().getById(task.getId()));
        } catch (DAOException e) {
            Helper.setErrorMessage(ERROR_LOADING_ONE, new Object[] {ObjectType.TASK.getTranslationSingular(), task.getId() }, logger, e);
            return -1;
        }
    }
    ...
    /**
     * Return name of digitization profile of given task if it has one and empty String else.
     * @param task TaskDTO for which the digitization profile is returned
     * @return name of digitization profile of given task if it has one and empty String else.
     */
    public String getDigitizationProfile(TaskDTO task) {
        return JobService.getDigitizationProfile(task.getTitle());
    }
    ...
    /**
     * Get first open correction comment of the given process.
     *
     * @param process
     *          process to get the correction comment for
     * @return optional correction comment
     */
    public Optional<Comment> getOpenCorrectionComment(Process process) {
        return ServiceManager.getCommentService().getAllCommentsByProcess(process).stream()
                .filter(c -> CommentType.ERROR.equals(c.getType()) && !c.isCorrected())
                .findFirst();
    }
    ...
    /**
     * Get the process for the given task as ProcessDTO.
     * @param task Task to get ProcessDTO for
     * @return process as org.kitodo.production.dto.ProcessDTO
     */
    public ProcessDTO findProcess(Task task) {
        try {
            return ServiceManager.getProcessService().findById(task.getProcess().getId());
        } catch (DataException e) {
     ...
     * @param task the task to load the process' property containing the container String from
     * @return List of containers, represented as List of Strings containing the container code and container type
     */
    public List<List<String>> getProcessContainers(Task task) {
        return ServiceManager.getProcessService().getProcessContainers(findProcess(task));
    }
