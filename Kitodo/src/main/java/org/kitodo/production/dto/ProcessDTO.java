...
public class ProcessDTO extends BaseTemplateDTO {
     ...
     * Set priority.
     *
     * @param priority as org.kitodo.data.database.enums.ProcessPriority
     */
    ...
    /**
     * Return value of metadata with given name if this process contains such metadata.
     * @param metadataName name of metadata for which the value will be returned
     * @return value of metadata or empty String if this process does not contain metadata with this name
     */
    public String getMetadataValue(String metadataName) {
        Optional<Metadata> metadataElement = this.metadata
                .stream().filter(m -> m.getKey().equals(metadataName) && m instanceof MetadataEntry).findFirst();
        if (metadataElement.isPresent()) {
            return ((MetadataEntry) metadataElement.get()).getValue();
        } else {
            return "";
        }
    }
