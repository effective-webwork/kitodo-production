...
public class SearchResultGeneration {
    ...
    private ProcessPriority processPriority = null;
    ...
    /**
     * Constructor including process priority restriction.
     *
     * @param filter
     *            String
     * @param showClosedProcesses
    ...
    public BoolQueryBuilder getQueryForFilter(ObjectType objectType) {
        ...
        if(Objects.nonNull(this.processPriority)) {
            query.must(ServiceManager.getProcessService().getQueryForProcessPriority(this.processPriority.getValue()));
        }
        return query;
    }
    ...
    private void prepareRow(int rowCounter, HSSFSheet sheet, ProcessDTO processDTO) {
        ...
        row.createCell(2).setCellValue(processDTO.getCreationDate().toString());
