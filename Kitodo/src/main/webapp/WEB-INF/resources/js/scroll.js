...
function scrollToSelectedTreeNode(source) {
    ...
    if (source !== 'logicalStructure' && selectedLogicalNode.length === 1 && logicalStructure.length) {
        logicalStructure.animate({
            scrollTop: selectedLogicalNode.position().top - logicalStructure.height()/2
        }, 180, null, null);
    }
    if (source !== 'physicalStructure' && selectedPhysicalNode.length === 1 && physicalStructure.length) {
