...
var secondSectionFirstColumn = $('#paginationPanel');
var firstSectionSecondColumn = $('#metadataPanel');
var secondSectionSecondColumn = $('#commentsPanel');
...
$(document).ready(function() {
    ...
    setSizes();
    // show second section (comments) by default if comments exist
    if (secondSectionSecondColumn.data('comment-number') >= 0) {
        toggleSecondSectionSecondColumn();
    }
    // show second section (pagination panel/docket controls) by default
...
function setSectionHeightFirstColumn() {
    // display only min-height in Vecteur
    if (secondSectionFirstColumn.hasClass(COLLAPSED)) {
        ...
        firstSectionFirstColumn.height(wrapper.height() - secondSectionFirstColumn.data('min-height') - 2 * HEADING_HEIGHT - (parseInt(firstColumn.css('padding-top')) / 2));
        secondSectionFirstColumn.height(secondSectionFirstColumn.data('min-height'));
    }
}
...
function toggleSecondSection(firstSection, secondSection, firstSectionToggler, secondSectionHeight) {
    secondSection.height(secondSectionHeight);
