...
public class TaskType extends BaseType<Task> {
    ...
    Map<String, Object> getJsonObject(Task task) {
        ...
        jsonObject.put(TaskTypeField.PROCESS_PRIORITY.getKey(), getPriority(task));
    ...
    private int getPriority(Task task) {
        if (Objects.nonNull(task.getProcess()) && Objects.nonNull(task.getProcess().getPriority())) {
            return task.getProcess().getPriority().getValue();
        }
        return 1;
    }
