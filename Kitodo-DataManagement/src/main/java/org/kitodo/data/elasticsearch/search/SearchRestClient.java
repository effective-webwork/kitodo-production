...
public class SearchRestClient extends KitodoRestClient {
    ...
    SearchHits getDocument(String type, QueryBuilder query, SortBuilder sort, Integer offset, Integer size)
        ...
        if (Objects.nonNull(sort)) {
            ...
            // tasks that are sorted by process priority also need to be sorted by process age (e.g. "creationDate")
            if ("task".equals(type) && "processForTask.priority".equals(((FieldSortBuilder) sort).getFieldName())) {
                if (SortOrder.DESC.equals(sort.order())) {
                    sourceBuilder.sort(SortBuilders.fieldSort("processForTask.creationDate").order(SortOrder.ASC));
                } else {
                    sourceBuilder.sort(SortBuilders.fieldSort("processForTask.creationDate").order(SortOrder.DESC));
                }
        ...
        if (Objects.nonNull(size)) {
            ...
            // FIXME
            //  It is currently not possible to query a dataset larger than 10,000 objects reliably.
            //  This is just a temporary solution!
            //  If the total number of objects in index is larger than 10,000 not all objects can the retrieved.
            //  This needs to be rewritten to support requests for a larger dataset.
            //  (Elasticsearch does not allow requests for more than 10,000 objects by default.)
