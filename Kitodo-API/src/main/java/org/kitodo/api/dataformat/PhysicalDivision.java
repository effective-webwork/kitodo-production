...
public class PhysicalDivision extends Division<PhysicalDivision> {
    ...
    public PhysicalDivision(Workpiece workpiece) {
        logicalDivisions = new LinkedList<>();
        if (Objects.nonNull(workpiece)) {
            metsDivReferrerId = ID_PREFIX + String.format("%04d", getMaxId(workpiece) + 1);
        }
    }
    ...
    private int getMaxId(Workpiece workpiece) {
        return workpiece.getAllPhysicalDivisions().stream()
                .filter(physicalDivision -> physicalDivision.getDivId().startsWith(ID_PREFIX))
                .filter(element -> StringUtils.isNumeric(element.getDivId().replace(ID_PREFIX, "")))
                .max(Comparator.comparingInt(o -> Integer.parseInt(o.getDivId().replace(ID_PREFIX, ""))))
                .map(physicalDivision -> Integer.parseInt(physicalDivision.getDivId().replace(ID_PREFIX, "")))
                .orElse(0);
    }
